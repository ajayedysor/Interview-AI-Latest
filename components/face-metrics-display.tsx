...
export function FaceMetricsDisplay({
  data: metrics,
  isActive,
  isLoading,
  error,
  ...
  return (
    <div className={`bg-black/70 backdrop-blur-sm rounded-lg p-3 text-xs space-y-2 min-w-[160px] ${className}`}>
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <span className={(metrics?.faceCount || 0) > 1 ? "text-red-400 font-semibold" : "text-gray-200"}>
          {metrics?.faceCount || 0} {(metrics?.faceCount || 0) > 1 && "⚠️"}
        </span>
      </div>
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <span className={`capitalize ${metrics?.headPosition === "center" ? "text-emerald-400" : "text-yellow-400"}`}>
          {getHeadPositionIcon()} {metrics?.headPosition || "center"}
        </span>
      </div>
    </div>
  );
