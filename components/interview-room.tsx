...
export function InterviewRoom({ 
  ...
  const handleCallEnd = (endedReason?: string) => {
    ...
    // In ElevenLabs, we don't have the same reason codes as Vapi, 
    // but we can map the end of session to completion.
  ...
  const isConfigured = true; // ElevenLabs uses signed URL from server
  const configError = elevenLabsError;
  const volumeLevel = isSpeaking ? 0.5 : 0; // Placeholder for ripple effect

  // Reset reconnectAttempts after a successful reconnect (when isCalling becomes true)
  useEffect(() => {
  ...
  const { 
    ...
    onViolation: (violation) => {
      if (isCalling) {
        sendRuntimeMessage(violation.message);
      }
    },
    noFaceThreshold: 2,
    lookAwayThreshold: 3,
...
useEffect(() => {
  if (isVideoOn && videoStream && hasPermissions && isProcessorReady) {
    // videoStream is already video-only, no need to create new MediaStream
    startDetection(videoStream)
  } else {
    stopDetection()
  }
    ...
    const handleVisibility = async () => {
      if (document.visibilityState === "visible" && !reconnectingNow) {
        ...
        try {
          ...
          await startDetection && startDetection(videoOnlyStream);
        ...
        <div className="relative bg-black rounded-3xl overflow-hidden">
          {isVideoOn ? (
            <>
              ...
              />
              <FaceMetricsDisplay data={processedData} isActive={isFaceDetectionActive} />
            </>
          ) : (
